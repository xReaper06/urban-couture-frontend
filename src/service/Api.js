...
export default () => {
  const instance = axios.create({
    baseURL: 'http://localhost:3080/api', // Make sure your server is running on this URL
  });

  instance.interceptors.request.use(
    (config) => {
      const token = sessionStorage.getItem('accessToken'); // Assuming you store the token in your store
      if (token) {
    ...
    async (error) => {
      ...
      if (error.response && error.response.status === 401 && !originalRequest._retry) {
        ...
        try {
          ...
          const refreshResponse = await axios.post('http://localhost:3080/api/Token', { token:refreshToken });
