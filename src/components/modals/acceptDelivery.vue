<template>
    <div>
        <div class="modal mb-5 pb-5" v-if="props.showAcceptModal">
  ...
  <rate-this-prod :show-rate-modal="showRateModal" @close-rate-modal="closeRateModal" :product_id="product_id"/>
    </div>
</template>

<script setup>
import { ref,defineProps, defineEmits,watchEffect} from "vue";
import rateThisProd from "./rateThisProd.vue";
import AuthenticationService from '@/service/AuthenticationService.js'
  ...
  const confirmDelivery = async()=>{
    try {
      const response = await AuthenticationService.confirmDelivery({
  ...
  const emits = defineEmits(["closeAcceptModal","puchaseHistory"]);
  
  const closeModal = () => {
    emits("closeAcceptModal");
  };
  const purchaseHistory = ()=>{
    emits("puchaseHistory")
  }
